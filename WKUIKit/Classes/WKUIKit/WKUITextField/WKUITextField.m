...
// 私有的类，专用于实现 WKUITextFieldDelegate，避免 self.delegate = self 的写法（
@interface _WKUITextFieldDelegator : NSObject <WKUITextFieldDelegate, UIScrollViewDelegate>

@property(nonatomic, weak) WKUITextField *textField;
- (void)handleTextChangeEvent:(WKUITextField *)textField;
@end

@interface WKUITextField ()

@property(nonatomic, strong) _WKUITextFieldDelegator *delegator;
@end
...
- (void)didInitialize {
    self.delegator = [[_WKUITextFieldDelegator alloc] init];
...
- (void)fireTextDidChangeEventForTextField:(WKUITextField *)textField {
...
@implementation _WKUITextFieldDelegator

#pragma mark - <WKUITextFieldDelegate>

- (BOOL)textField:(WKUITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    if (textField.maximumTextLength < NSUIntegerMax) {
        ...
        if (NSMaxRange(range) > textField.text.length) {
            ...
            if (range.length > 0) {
                UITextRange *textRange = [self.textField wkui_convertUITextRangeFromNSRange:range];
                [self.textField replaceRange:textRange withText:string];
            }
            return NO;
        }
        
        NSUInteger rangeLength = textField.shouldCountingNonASCIICharacterAsTwo ? [textField.text substringWithRange:range].wkui_lengthWhenCountingNonASCIICharacterAsTwo : range.length;
        if ([textField lengthWithString:textField.text] - rangeLength + [textField lengthWithString:string] > textField.maximumTextLength) {
            ...
            if (substringLength > 0 && [textField lengthWithString:string] > substringLength) {
                NSString *allowedText = [string wkui_substringAvoidBreakingUpCharacterSequencesWithRange:NSMakeRange(0, substringLength) lessValue:YES countingNonASCIICharacterAsTwo:textField.shouldCountingNonASCIICharacterAsTwo];
...
- (void)handleTextChangeEvent:(WKUITextField *)textField {
    ...
    if (!textField.markedTextRange) {
        if ([textField lengthWithString:textField.text] > textField.maximumTextLength) {
            textField.text = [textField.text wkui_substringAvoidBreakingUpCharacterSequencesWithRange:NSMakeRange(0, textField.maximumTextLength) lessValue:YES countingNonASCIICharacterAsTwo:textField.shouldCountingNonASCIICharacterAsTwo];
            
            if ([textField.delegate respondsToSelector:@selector(textField:didPreventTextChangeInRange:replacementString:)]) {
                [textField.delegate textField:textField didPreventTextChangeInRange:textField.wkui_selectedRange replacementString:nil];
            }
        }
    }
}
