...
@implementation UITextView (WKUI)
...
- (void)wkui_scrollRangeToVisible:(NSRange)range {
    ...
    UITextRange *textRange = [self wkui_convertUITextRangeFromNSRange:range];
    if (!textRange) return;
...
- (void)_scrollRectToVisible:(CGRect)rect animated:(BOOL)animated {
    // scrollEnabled 为 NO 时可能产生不合法的 rect 值 https://github.com/Tencent/wkui_iOS/issues/205
