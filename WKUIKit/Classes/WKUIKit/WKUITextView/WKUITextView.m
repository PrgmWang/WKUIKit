...
/// 系统 textView 默认的字号大小，用于 placeholder 默认的文字大小。实测得到 。
const CGFloat kSystemTextViewDefaultFontPointSize = 12.0f;

/// 当系统的 textView.textContainerInset 为 UIEdgeInsetsZero 时，文字与 textView 边缘的间距。实测得到（在输入框font大于13时准确，小于等于12时，y有-1px的偏差）。
const UIEdgeInsets kSystemTextViewFixTextInsets = {0, 5, 0, 5};

// 私有的类，专用于实现 WKUITextViewDelegate，避免 self.delegate = self 的写法（以前是 WKUITextView 自己实现了 delegate）
@interface _WKUITextViewDelegator : NSObject <WKUITextViewDelegate>

@property(nonatomic, weak) WKUITextView *textView;
@end

@interface WKUITextView ()

@property(nonatomic, assign) BOOL debug;
@property(nonatomic, assign) BOOL postInitializationMethodCalled;
@property(nonatomic, strong) _WKUITextViewDelegator *delegator;
...
@property(nonatomic, strong) WKUILabel *placeholderLabel;

@end

@implementation WKUITextView
...
- (void)didInitialize {
    ...
    self.delegator = [[_WKUITextViewDelegator alloc] init];
    ...
    self.placeholderLabel = [[WKUILabel alloc] init];
...
- (void)_WKUI_setTextForShouldChange:(NSString *)text {
...
- (void)handleTextChanged:(id)sender {
    WKUITextView *textView = nil;
    if ([sender isKindOfClass:[NSNotification class]]) {
        id object = ((NSNotification *)sender).object;
        if (object == self) {
            textView = (WKUITextView *)object;
        }
    } else if ([sender isKindOfClass:[WKUITextView class]]) {
        textView = (WKUITextView *)sender;
    }
    ...
    if (!textView.markedTextRange && [textView lengthWithString:textView.text] > textView.maximumTextLength) {
        NSString *finalText = [textView.text wkui_substringAvoidBreakingUpCharacterSequencesWithRange:NSMakeRange(0, textView.maximumTextLength) lessValue:YES countingNonASCIICharacterAsTwo:textView.shouldCountingNonASCIICharacterAsTwo];
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [textView wkui_scrollCaretVisibleAnimated:NO];
    });
}
...
- (NSUInteger)lengthWithString:(NSString *)string {
    return self.shouldCountingNonASCIICharacterAsTwo ? string.wkui_lengthWhenCountingNonASCIICharacterAsTwo : string.length;
}
...
@implementation _WKUITextViewDelegator

#pragma mark - <WKUITextViewDelegate>

- (BOOL)textView:(WKUITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    ...
    if (textView.maximumTextLength < NSUIntegerMax) {
        ...
        if (NSMaxRange(range) > textView.text.length) {
            ...
            if (range.length > 0) {
                UITextRange *textRange = [self.textView wkui_convertUITextRangeFromNSRange:range];
                [self.textView replaceRange:textRange withText:text];
            }
            return NO;
        }
        
        NSUInteger rangeLength = textView.shouldCountingNonASCIICharacterAsTwo ? [textView.text substringWithRange:range].wkui_lengthWhenCountingNonASCIICharacterAsTwo : range.length;
        ...
        if (textWillOutofMaximumTextLength) {
            ...
            if (substringLength > 0 && [textView lengthWithString:text] > substringLength) {
                NSString *allowedText = [text wkui_substringAvoidBreakingUpCharacterSequencesWithRange:NSMakeRange(0, substringLength) lessValue:YES countingNonASCIICharacterAsTwo:textView.shouldCountingNonASCIICharacterAsTwo];
                if ([textView lengthWithString:allowedText] <= substringLength) {
                    [textView _WKUI_setTextForShouldChange:[textView.text stringByReplacingCharactersInRange:range withString:allowedText]];
