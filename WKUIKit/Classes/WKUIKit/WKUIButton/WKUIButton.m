...
const CGFloat WKUIButtonCornerRadiusAdjustsBounds = -1;

@interface WKUIButton ()

@end

@implementation WKUIButton

+ (instancetype)wkui_buttonWithType:(UIButtonType)buttonType title:(NSString *)title titleFont:(UIFont *)titleFont normalTitleColor:(UIColor *)normalTitleColor selectedTitleColor:(UIColor *)selectedTitleColor normalImage:(NSString *)normalImage selectedImage:(NSString *)selectedImage{
    WKUIButton *btn = [super buttonWithType:buttonType];
    if (btn) {
        if (title) {
            [btn setTitle:title forState:UIControlStateNormal];
        }
        if (titleFont) {
            btn.normalTitleFont = titleFont;
...
- (instancetype)initWithFrame:(CGRect)frame title:(NSString *)title titleFont:(UIFont *)titleFont normalTitleColor:(UIColor *)normalTitleColor selectedTitleColor:(UIColor *)selectedTitleColor normalImage:(NSString *)normalImage selectedImage:(NSString *)selectedImage{
    ...
    if (self) {
        ...
        if (titleFont) {
            self.normalTitleFont = titleFont;
...
- (void)didInitialize {
    ...
    self.imagePosition = WKUIButtonImagePositionLeft;
}

// 系统访问 self.imageView 会触发 layout，而私有方法 _imageView 则是简单地访问 imageView，所以在 WKUIButton layoutSubviews 里应该用这个方法
- (UIImageView *)_wkui_imageView {
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    switch (self.imagePosition) {
        case WKUIButtonImagePositionTop:
        case WKUIButtonImagePositionBottom: {
            ...
            if (isImageViewShowing) {
                ...
                imageSize.width = MIN(imageSize.width, imageLimitWidth);// WKUIButton sizeThatFits 时 self._imageView 为 nil 但 self.imageView 有值，而开启了 Bold Text 时，系统的 self.imageView sizeThatFits 返回值会比没开启 BoldText 时多 1pt（不知道为什么文字加粗与否会影响 imageView...），从而保证开启 Bold Text 后文字依然能完整展示出来，所以这里应该用 self.imageView 而不是 self._imageView
        ...
        case WKUIButtonImagePositionLeft:
        case WKUIButtonImagePositionRight: {
            // 图片和文字水平排版时，高度以文字或图片的最大高度为最终高度
            // 注意这里有一个和系统不一致的行为：当 titleLabel 为多行时，系统的 sizeThatFits: 计算结果固定是单行的，所以当 WKUIButtonImagePositionLeft 并且titleLabel 多行的情况下，WKUIButton 计算的结果与系统不一致
            ...
            if (isImageViewShowing) {
                ...
                imageSize.height = MIN(imageSize.height, imageLimitHeight);// WKUIButton sizeThatFits 时 self._imageView 为 nil 但 self.imageView 有值，而开启了 Bold Text 时，系统的 self.imageView sizeThatFits 返回值会比没开启 BoldText 时多 1pt（不知道为什么文字加粗与否会影响 imageView...），从而保证开启 Bold Text 后文字依然能完整展示出来，所以这里应该用 self.imageView 而不是 self._imageView
                imageTotalSize = CGSizeMake(imageSize.width + UIEdgeInsetsGetHorizontalValue(self.imageEdgeInsets), imageSize.height + UIEdgeInsetsGetVerticalValue(self.imageEdgeInsets));
            }
...
- (void)layoutSubviews {
    ...
    if (self.cornerRadius == WKUIButtonCornerRadiusAdjustsBounds) {
    ...
    if (isImageViewShowing) {
        ...
        CGSize imageSize = self._wkui_imageView.image ? [self._wkui_imageView sizeThatFits:imageLimitSize] : self.currentImage.size;
    ...
    if (isImageViewShowing) {
        makesureBoundsPositive(self._wkui_imageView);
    }
    if (isTitleLabelShowing) {
        makesureBoundsPositive(self.titleLabel);
    }
    
    if (self.imagePosition == WKUIButtonImagePositionTop || self.imagePosition == WKUIButtonImagePositionBottom) {
        ...
        if (self.imagePosition == WKUIButtonImagePositionTop) {
        ...
        if (isImageViewShowing) {
            ...
            self._wkui_imageView.frame = imageFrame;
        }
        if (isTitleLabelShowing) {
    ...
    } else if (self.imagePosition == WKUIButtonImagePositionLeft || self.imagePosition == WKUIButtonImagePositionRight) {
        ...
        if (self.imagePosition == WKUIButtonImagePositionLeft) {
            switch (self.contentHorizontalAlignment) {
                case UIControlContentHorizontalAlignmentLeft:
                    imageFrame = isImageViewShowing ? CGRectSetX(imageFrame, contentEdgeInsets.left + self.imageEdgeInsets.left) : imageFrame;
...
- (void)setImagePosition:(WKUIButtonImagePosition)imagePosition {
...
- (void)setCornerRadius:(CGFloat)cornerRadius {
    ...
    if (cornerRadius != WKUIButtonCornerRadiusAdjustsBounds) {
...
-(void)setSelected:(BOOL)selected{
    ...
    if (selected) {
        ...
        if (_selectedTitleFont ) {
            self.titleLabel.font = _selectedTitleFont;
        }
    }else if (_normalBackgroundColor && !selected){
